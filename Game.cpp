...
void Game::keyDown(int vk) {
    switch (state) {
    case State::MENU:
        if (vk == VK_RETURN || vk == VK_SPACE) state = State::PLAYING;
        break;

    case State::PLAYING:
        if (vk == VK_LEFT  || vk == 'A') keyLeft  = true;
        if (vk == VK_RIGHT || vk == 'D') keyRight = true;
        if (vk == VK_UP    || vk == 'W' || vk == VK_SPACE) {
            if (!keyUp) { player.jump(); keyUp = true; }
        }
...
void Game::keyUp(int vk) {
    if (vk == VK_LEFT  || vk == 'A') keyLeft  = false;
    if (vk == VK_RIGHT || vk == 'D') keyRight = false;
    if (vk == VK_UP    || vk == 'W' || vk == VK_SPACE) keyUp = false;
}
...
void Game::update(float dt) {
    ...
    if (!keyLeft && !keyRight) player.stopMoving();
                    ...
                    int offX, int offY)
{
    ...
    int w = max(2, (int)(ww * scaleX));
    int h = max(2, (int)(wh * scaleY));
...
void Game::render(HDC hdc, int clientW, int clientH) {
    ...
    if (state == State::MENU) {
        ...
        drawString(hdcBuffer, "GAME  OVER",         clientW/2-100, clientH/2-80, COL_RED,    36);
        drawString(hdcBuffer, "Score: " + std::to_string(player.getScore()),
                                                    clientW/2-70,  clientH/2,    COL_WHITE,  20);
        drawString(hdcBuffer, "Press SPACE to exit",clientW/2-110, clientH/2+50, COL_YELLOW, 18);
